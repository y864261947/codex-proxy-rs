...
读取成功记录只访问 Store，不读取凭据或请求上游。历史按渠道隔离、序号降序做有界游标分页，不以墙上时钟排序。差异保留查询时的配置语境，旧连接版本仍可查看但不能直接添加，历史页面一律只读。最新结果的显式选择只添加编辑草稿，最终保存继续复用渠道更新事务、审计和快照发布。未发现的旧模型不自动删除，发现结果不扩展权限或推断能力/价格。当前不维护失败尝试历史、自动清理或定时同步。
