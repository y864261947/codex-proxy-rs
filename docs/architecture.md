...
手动渠道发现是独立的 Admin 用例，经 `ChannelProviderAdmin` 调用 Provider 自有协议适配；它不改变只读运行目录的行为。Provider 仅返回已配置和发现的模型 ID 集合，Admin 检查连接版本、预留发现序号并计算差异。Store 在独立观测表中保存每渠道最新成功结果；短事务锁定渠道行并复核连接版本，只接受比已保存记录更大的序号，避免旧查询迟到覆盖新成功结果。网络请求不在事务内，观测保存不改变配置版本、配置审计或运行快照；失败保留旧成功记录，删除渠道级联删除记录。

读取成功记录只访问 Store，不读取凭据或请求上游。差异保留查询时的配置语境，旧连接版本仍可查看但不能直接添加。显式选择只添加编辑草稿，最终保存继续复用渠道更新事务、审计和快照发布。未发现的旧模型不自动删除，发现结果不扩展权限或推断能力/价格。当前仅保存最新成功结果，不维护多版本历史、失败尝试历史或定时同步。
