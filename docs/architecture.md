...
Core `identity` 拥有 `AccountGroupId` 与 `SourceId`，`policy` 拥有来源偏好和分组覆盖值对象；策略不能反向依赖路由执行。既有 account/routing 导出路径重导出同一类型，不建立第二套状态。

Core `catalog::ModelCatalogReader` 从同一已发布运行快照投影模型、来源配置与能力证据；Admin 负责查询校验、过滤和分页，API 仅适配管理 wire。目录读取不访问 Provider、Store 或上游，不能刷新凭据、发现模型或产生测试流量。渠道与号池分别保留稳定来源身份，缺少能力不推断支持，配置就绪与运行健康/授权保持独立。

手动渠道发现是独立的 Admin 用例，经 `ChannelProviderAdmin` 调用 Provider 自有协议适配；它不改变只读运行目录的行为。Provider 仅返回已配置和发现的模型 ID 集合，Admin 检查连接版本、预留发现序号并计算差异。Store 在独立观测表中按渠道与序号追加成功记录；短事务锁定渠道行并复核连接版本，只接受比已保存记录更大的序号，避免旧查询迟到覆盖新成功结果。最新结果由同一表最大序号投影，不引入第二份最新状态。网络请求不在事务内，观测保存不改变配置版本、配置审计或运行快照；失败保留旧成功记录，删除渠道级联删除全部记录。

读取成功记录只访问 Store，不读取凭据或请求上游。历史按渠道隔离、序号降序做有界游标分页，不以墙上时钟排序。差异保留查询时的配置语境，旧连接版本仍可查看但不能直接添加，历史页面一律只读。最新结果的显式选择只添加编辑草稿，最终保存继续复用渠道更新事务、审计和快照发布。未发现的旧模型不自动删除，发现结果不扩展权限或推断能力/价格。当前不维护完整失败尝试历史或自动清理。

渠道定时发现默认关闭，配置权威在 PostgreSQL，复用渠道保存、版本递增、审计与发布事务。Admin 的独立模型发现 Worker 由 Host 监督和续租，不私建后台执行器；每轮只领取一个启用且到期的渠道，复用同一发现用例。Store 单条加锁查询领取并提前推进到期时间，重启不补跑积压周期；失败只等下个周期，不产生无限立即重试。保存渠道重新计时并清空最近尝试状态，已有成功历史不变。连接版本保护发现记录，连接版本与领取标识共同保护最近完成状态。关闭计划不能撤回已领取或已发送的查询，旧版本结果拒绝保存。最近尝试状态不是第二份模型目录，也不作为路由健康或可用性证据；完成未确认时以成功历史进一步核对，观测写入不发布配置快照。迁移 `0013` 不替既有渠道启用计划；单副本运行边界不变。

两次发现比较由 Admin 拥有集合语义，Store 只按渠道与精确序号在单条查询内读取一对记录，API 只呈现元数据和差异。每条真实发现集合还原为新增与重合项的并集，不把查询时配置的未发现项计入；配置变更本身不能制造模型增减。基准序号必须小于目标，跨连接版本可以只读比较但必须提示配置语境不同。客户端跨页选择不会触发上游，也不提供从历史直接变更配置的旁路。

迁移 `0012` 将观测表主键扩展为渠道与序号，保留已有最后成功结果；更早被覆盖的记录无法追溯。此迁移需与新版应用配套部署，旧版单渠道 upsert 不兼容新主键，不能只回退应用二进制。
...
Key 关联接入分组后，以组的显式模型白名单和号池/渠道集合替代旧分组范围；客户和接入分组的启停及
独立限额进入同一运行快照。外部渠道是稳定 `ChannelId`，不能伪装成账号；候选冻结渠道配置版本，具体
Provider 持有凭据与协议。模型授权在对外别名映射之前检查，目录与调用共用权限。

号池和渠道共用来源选择策略。旧 Key 也在原账号权限内生成号池候选，只有真正未分池账号保留无来源
后备；固定账号诊断先求账号交集再路由。交集只收窄候选，不改变历史保存的 Key 权限快照。未分池索引
从账号目录派生并随快照复用，不在每次请求中扫描全目录。

全站、客户、接入分组和 Key 的准入以逻辑请求为单位；来源及具名共享配额以每次上游尝试为单位。
Store 用 Redis 原子检查该次准入的全部范围，来源等待者取消后仍清理迟到的 grant；流终态及取消释放
来源租约，RPM 记录保留。Core 编排来源、Store 执行协调、Host 管理有界 worker，Provider 的账号租约
继续独立生效。共享容量不合并健康：尝试反馈按冻结来源更新 circuit，无来源路径保留 Provider 范围。
健康读取失败沿用可重建状态 fail-open，来源容量协调失败则禁止发送上游；容量不足不计为来源故障。
...
- native continuation 固定创建它的 Provider、账号及已记录的具体来源；
